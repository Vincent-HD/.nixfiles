...
Item {
  ...
  function focusWindow(window) {
    try {
      const windowWorkspace = workspaceCache[window.workspaceId];
      let currentWorkspaceId = -1;
      for (var i = 0; i < workspaces.count; i++) {
        if (workspaces.get(i).isFocused) {
          currentWorkspaceId = workspaces.get(i).id;
          break;
        }
      }

      if (windowWorkspace && currentWorkspaceId !== -1 && windowWorkspace.id !== currentWorkspaceId) {
        Logger.d("NiriService", "Switching to workspace " + windowWorkspace.idx + " before focusing window " + window.id);
        Niri.dispatch(["focus-workspace", windowWorkspace.idx.toString()]);
        Qt.callLater(() => {
