...
| Later cleanup; stacked PRs; squash & resplit a wip stack | `jj-resplit-stack` | Preflight (stack vs `@`, N/yolo, bookmarks, typecheck TIP) → capture op id → squash blob onto BASE → peel slices → typecheck + added tests per rev |
| Unresolved jj conflicts after rebase/merge; “fix conflicts” | `jj-solve-conflict` | Capture op id, teleport with `jj new` onto the oldest conflict, resolve, squash with `--use-destination-message`, keep a running remaining-count |
