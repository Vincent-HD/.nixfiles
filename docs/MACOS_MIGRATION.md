...
| GIPHY CAPTURE, Keynote, Numbers, Pages | Mac App Store | keep | mas | Declared through `homebrew.masApps`. No cask and no nixpkgs build exists for any of them, so the App Store is the only source. | no |
...
| GarageBand, iMovie | Mac App Store | unmanaged | none | Dropped from the declaration 2026-09-13. No cask and no nixpkgs build exists, so no Nix owner is possible; `brew bundle cleanup` ignores App Store applications, so leaving them undeclared produces no drift. | yes |
...
3. GarageBand and iMovie are also absent, at the user's request, and are now unmanaged rather than
   declared. No Nix owner is possible for either. `brew bundle cleanup` ignores App Store
   applications, so an undeclared App Store application is not drift and does not block activation.
