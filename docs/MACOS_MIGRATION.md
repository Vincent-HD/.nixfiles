...
| Google Chrome | unmanaged | migrate | home-manager | Declared in `hm.guiApps` for Darwin (nixpkgs 152.0.7977.76 against manual 133.0.6943.54). The root-owned bundle needs a `sudo` removal, and Google's updater restored it once already; see TODO.md. | no |
