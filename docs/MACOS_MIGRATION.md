...
| GarageBand, iMovie | Mac App Store | remove | none | Dropped from the declaration and moved to the Trash on 2026-09-13. No cask and no nixpkgs build exists, so no Nix owner was possible; `brew bundle cleanup` ignores App Store applications, so neither being undeclared produces drift. Reclaimed 4.4 GB once the Trash is emptied. | yes |
