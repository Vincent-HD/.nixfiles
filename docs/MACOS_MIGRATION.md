...
- Mac App Store applications: GarageBand, GIPHY CAPTURE, iMovie, Keynote, Numbers, Pages, Tailscale.
  Bitwarden is no longer in this group; Home Manager owns it.
...
| GarageBand, GIPHY CAPTURE, iMovie, Keynote, Numbers, Pages, Tailscale | Mac App Store | keep | mas | Declared through `homebrew.masApps` in `darwin.homebrew`. Tailscale needs the App Store build's network extension; the rest have no nixpkgs build. | no |
...
| --- | --- | --- |
| Bitwarden | `hm.bitwarden` | Both hosts. Only Linux pins the SSH agent socket; the App Store copy is pending removal. |
...
### Tier 4 - Mac App Store (7 declared)

GarageBand, GIPHY CAPTURE, iMovie, Keynote, Numbers, Pages, and Tailscale, declared in
`homebrew.masApps`. See Phase 4 for why Bitwarden is not in this list.
...
Three root-owned bundles duplicate an application that Home Manager already owns. All three need
`sudo`, and each becomes safe to delete once the replacement launches:
...
1. Seven App Store applications are declared through `homebrew.masApps` in `darwin.homebrew`:
   GarageBand, GIPHY CAPTURE, iMovie, Keynote, Numbers, Pages, and Tailscale.
...
3. Declaring rather than leaving them to the App Store is the recommendation made here.
...
4. `brew bundle check` flags GarageBand, iMovie, and Tailscale as unmet on this machine. Those are
   pending App Store updates, not missing applications, and they cannot block activation:
