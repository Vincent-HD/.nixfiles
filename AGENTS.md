...
AGENTS.md                              # This file
modules/
  agents.nix                           # Shared Agent Skills, Cursor MCPs, Papercuts, and RTK
...
- **Add cross-client Agent Skills through `custom.agentSetup.skills` in `modules/agents.nix` (or a
...
- **Keep MCP definitions explicit in each client's native schema.** Maintain Cursor in
  `modules/agents.nix`, Codex in `modules/codex.nix`, and OpenCode in
  `modules/coding/opencode.nix`. Repeating the small server list is preferred over a normalizing
